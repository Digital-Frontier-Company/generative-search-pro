...
serve(async (req) => {
  ...
  try {
    ...
    // Create the enhanced content analysis functions
    ...
    const { error } = await supabaseClient.rpc('exec', { sql: createFunctions });
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Analysis functions created successfully' }),
      { 
        status: 200, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      }
