...
const AIVisibilityScore = () => {
  ...
  const [metrics, setMetrics] = useState<VisibilityMetric[]>([{
    ...
    recommendations: ["Structure content with clear headings and lists", "Answer specific questions directly", "Use FAQ schema markup"]
  }, {
    ...
    recommendations: ["Create comprehensive, authoritative content", "Use structured data markup", "Optimize for question-based queries"]
  }, {
    label: "Voice Search",
    score: 0,
    change: 0,
    ...
    recommendations: ["Target conversational keywords", "Create FAQ-style content", "Optimize for local search"]
  }, {
    label: "Schema Markup",
    score: 0,
    change: 0,
    ...
    recommendations: ["Implement Article schema", "Add FAQ schema markup", "Use Organization schema"]
  }, {
    ...
    recommendations: ["Use clear heading hierarchy", "Break content into digestible sections", "Add table of contents"]
  }]);
  ...
  const handleAnalyzeDomain = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('analyze-schema-patch', {
        body: {
      ...
      if (data.success) {
        ...
        // Generate insights
        const newInsights = [`Your AI visibility score of ${visibilityScore} is ${visibilityScore >= 70 ? 'above' : visibilityScore >= 40 ? 'at' : 'below'} industry average`, `Featured snippets show the highest potential for improvement`, `Schema markup implementation could boost your score by up to 20 points`, `Voice search optimization is becoming increasingly important for AI visibility`];
  ...
  return <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center">
        <h1 className="text-3xl mb-2 flex items-center justify-center gap-2 text-lime-400 font-extrabold">
          <Eye className="w-8 h-8 text-blue-600 bg-violet-950" />
          AI Visibility Score
        </h1>
        <p className="text-lime-300 text-base font-bold">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-center text-lime-400 font-extrabold">
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            <Input placeholder="Enter domain (e.g., example.com)" value={domain} onChange={e => setDomain(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAnalyzeDomain()} className="flex-1" />
      ...
      {overallScore > 0 && <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-gray-600 mb-4">
                  {overallScore >= 70 ? 'Excellent! Your content is highly visible to AI search engines.' : overallScore >= 40 ? 'Good foundation with significant opportunities for improvement.' : 'Your AI visibility needs immediate attention and optimization.'}
                </p>
                
                {/* Key Insights */}
                {insights.length > 0 && <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-medium text-blue-900 mb-2">Key Insights</h4>
                    <ul className="space-y-1">
                      {insights.map((insight, index) => <li key={index} className="text-sm text-blue-800 flex items-start gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {metrics.slice(0, 3).map((metric, index) => <Card key={index}>
              ...
              {metrics.map((metric, index) => <Card key={index}>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="space-y-1">
                        {metric.recommendations.map((rec, i) => <li key={i} className="text-sm text-gray-600 flex items-start gap-2">
          ...
          <TabsContent value="competitors" className="space-y-6">
            <Card>
              ...
              <CardContent>
                {competitorData.length > 0 ? <div className="space-y-4">
                    {/* Your Domain */}
                    ...
                    {/* Competitors */}
                    {competitorData.map((competitor, index) => <div key={index} className="p-4 border rounded-lg">
                        <div className="flex items-center justify-between mb-3">
          ...
          <TabsContent value="trends" className="space-y-6">
            <Card>
              ...
              <CardContent>
                {trendData.length > 0 ? <div className="space-y-6">
                    {/* Trend Summary */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {trendData.map((data, index) => <div key={index} className="flex items-center gap-4">
