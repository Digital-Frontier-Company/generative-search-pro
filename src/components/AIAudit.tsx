...
const AIAudit = () => {
  ...
  useEffect(() => {
    ...
    const aiFindings: AuditFinding[] = [{
      ...
      description: (analysis as any).schema_count > 0 ? `${(analysis as any).schema_count} schema types detected` : 'No structured data found - critical for AI visibility',
      impact: 'high',
      ...
      description: (analysis as any).meta_description ? 'Meta description is present' : 'Missing or inadequate meta description',
      impact: 'medium',
      category: 'content',
      ...
      description: (analysis as any).heading_structure?.h1_count === 1 ? 'Proper H1 structure detected' : 'Multiple or missing H1 tags detected',
      impact: 'medium',
      category: 'content',
  ...
  return <div className="max-w-6xl p-6 space-y-6 px-0 py-0 mx-0 my-[74px]">
      <div className="text-center">
        <h1 className="text-3xl mb-2 flex items-center justify-center gap-2 font-extrabold text-lime-400">
          <Bot className="w-8 h-8 text-blue-600 bg-violet-950 rounded-lg" />
          AI SEO Audit
        </h1>
      ...
      {/* Input Section */}
      <Card>
        <CardHeader className="mx-[11px] my-[11px]">
          <CardTitle className="flex items-center gap-2 text-center text-lime-400 font-extrabold text-2xl">
            <Search className="w-5 h-5" />
            Domain Analysis
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 mx-[11px] my-[11px] px-0">
          <div className="flex gap-4 mx-[3px]">
            <Input placeholder="Enter domain (e.g., example.com)" value={domain} onChange={e => setDomain(e.target.value)} onKeyPress={e => e.key === 'Enter' && runAudit()} className="flex-1 mx-0" />
